#--------------------------------------------------------------------------
# Topic 2: part I simulation for Scenario I and Model I 
# the data has been generated anad analysed using TPA Laplace with log-link function 
#--------------------------------------------------------------------------
# libraries 
library(truncnorm)  
library(nloptr)
...
tpa_data <- function(n, theta_c, F0 = F0_La, QF = QF_La, 
                     g = glog, g.inv = glog.inv)
{
  x <-  rtruncnorm(n, a = 0, b = 2, mean =  0, sd = 2)
  eta <- glog.inv(Model(x)$beta1) 
...
do_sim <- function(tau, n, theta_c, h, m, ktype){
  ...
  # true values
  eta <- glog.inv(Model(fit$x0)$beta1)
