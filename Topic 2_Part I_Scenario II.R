#--------------------------------------------------------------------------
# Topic 2: part I simulation for Scenario II and Model I 
# the data has been generated and  analysed using TPA Laplace with 
# logit type link  function 
#--------------------------------------------------------------------------
# libraries 
library(truncnorm)  
library(nloptr)
...
#' @g : link function 
#' @f0: pdf for the reference distribution  
#' @F0: CDF for the reference distribution 
# density
...
# n <- 100
# set.seed(1235)
# test.data <- tpa_data(n=n, theta_c =  0.14 )
# y <- test.data$y
# x <- test.data$x
# d <- test.data$d
# sum(d==0)/n
...
local_nll <- function(beta,  y, d, x,  ker.w = NULL) {
  ...
  p1 = 1  #local linear 
  eta  <- glogit.inv(x%*%as.vector(beta[1:(p1+1)]))
  ...
  if(is.null(ker.w)){
    logL <- (d*log(dtpa(y, eta, phi, alpha, f0_ALa, glogit)) + 
               (1-d)*log(ptpa(y, eta, phi, alpha, F0_ALa, glogit, lower.tail = FALSE)))
  } else { logL <- (d*log(dtpa(y, eta, phi, alpha, f0_ALa, glogit)) + 
                      (1-d)*log(ptpa(y, eta, phi, alpha, F0_ALa, glogit, lower.tail = FALSE)))*ker.w
  }
...
{  
  ...
  for (j in 1:length(h)) {
    ...
    for(i in 1:m){
      ...
      etahat[i,j] <- glogit.inv(fit$solution[1])
...
#                     theta_c = 0.14, h = c(0.15, 0.25),  m = 10, ktype = "ep")
